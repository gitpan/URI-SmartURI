# This Makefile is for the URI::SmartURI extension to perl.
#
...
#     ABSTRACT => q[Subclassable and hostless URIs]
#     AUTHOR => q[Rafael Kitover <rkitover@cpan.org>]
#     DIR => []
#     DISTNAME => q[URI-SmartURI]
#     NAME => q[URI::SmartURI]
#     NO_META => q[1]
#     PL_FILES => {  }
#     PREREQ_PM => { Scalar::Util=>q[0], File::Find::Rule=>q[0], URI=>q[0], ExtUtils::MakeMaker=>q[6.42], namespace::clean=>q[0], Test::More=>q[0], List::Util=>q[0], Class::C3::Componentised=>q[0], MRO::Compat=>q[0.09], List::MoreUtils=>q[0], Moose=>q[0] }
#     VERSION => q[0.026]
...
TO_INST_PM = lib/URI/SmartURI.pm

PM_TO_BLIB = lib/URI/SmartURI.pm \
...
DISTNAME = URI-SmartURI
DISTVNAME = URI-SmartURI-0.026
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW) \
	  lib/URI/SmartURI.pm $(INST_MAN3DIR)/URI::SmartURI.$(MAN3EXT) 
...
ppd :
	$(NOECHO) $(ECHO) '<SOFTPKG NAME="$(DISTNAME)" VERSION="0,026,0,0">' > $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <TITLE>$(DISTNAME)</TITLE>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <ABSTRACT>Subclassable and hostless URIs</ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Rafael Kitover &lt;rkitover@cpan.org&gt;</AUTHOR>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="Class-C3-Componentised" VERSION="0,0,0,0" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="ExtUtils-MakeMaker" VERSION="6,42,0,0" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="File-Find-Rule" VERSION="0,0,0,0" />' >> $(DISTNAME).ppd
	...
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="MRO-Compat" VERSION="0,09,0,0" />' >> $(DISTNAME).ppd
	...
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="namespace-clean" VERSION="0,0,0,0" />' >> $(DISTNAME).ppd
...
pm_to_blib : $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', '\''$(PM_FILTER)'\'')' -- \
	  lib/URI/SmartURI.pm blib/lib/URI/SmartURI.pm 
